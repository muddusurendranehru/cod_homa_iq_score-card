-- CREATE TABLES in COD_HOMA_IQ_SCORE Database
-- Handles: Complex names, secure passwords, India phone format (+91 9963721999)

-- Table 1: PATIENTS
CREATE TABLE homa_patients (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,  -- Handles complex/long names
    age INTEGER NOT NULL CHECK (age >= 0 AND age <= 150),
    sex VARCHAR(10) NOT NULL CHECK (sex IN ('male', 'female', 'other')),
    phone VARCHAR(15),  -- Handles +91 9963721999 format (India)
    email VARCHAR(255),
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);

-- Table 2: ASSESSMENTS (Health Parameters)
...
-- Table 3: USERS/STAFF (for login)
CREATE TABLE homa_users (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,  -- Handles complex names
    email VARCHAR(255) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,  -- Stores bcrypt hash (handles complex passwords)
    phone VARCHAR(15),  -- Handles +91 9963721999 format
...
CREATE INDEX idx_patient_phone ON homa_patients(phone);
